...
# load forecasting competition data. Data is saved in energy.csv.gz
...
def extract_data(data_dir):
    ...
    if not os.path.exists(GEFCom_zipfile):
        sys.exit(
            "Download GEFCom2014.zip from " +
            "https://www.dropbox.com/s/pqenrr2mcvl0hk9/GEFCom2014.zip?dl=0 " +
            "and save it to the '{}' directory.".format(data_dir))
    ...
    zip_ref = zipfile.ZipFile(
        os.path.join(GEFCom_dir, 'GEFCom2014-E_V2.zip'),
        'r')
    ...
    data = pd.read_excel(
        os.path.join(data_dir, 'GEFCom2014-E', 'GEFCom2014-E.xlsx'),
        parse_date='Date')

    # create timestamp variable from Date and Hour
    data['timestamp'] = data['Date'].add(
        pd.to_timedelta(data.Hour - 1, unit='h'))
