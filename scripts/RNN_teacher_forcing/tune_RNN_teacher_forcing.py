...
jcp = models.JobCreateParameters(
    ...
    custom_toolkit_settings=models.CustomToolkitSettings(
        command_line='python $AZ_BATCHAI_JOB_MOUNT_ROOT/resources/scripts/RNN_teacher_forcing.py \
        ...
        -l {0} -b {1} -T {2} -r {3} -a {4}'.format(parameters['LATENT_DIM_1'], parameters['BATCH_SIZE'], parameters['T'],
...
params = 'T={0} LATENT_DIM_1={1} BATCH_SIZE={2} LEARNING_RATE={3} ALPHA={4}'.format(T[best_run],
             latent_dim_1[best_run], batch_size[best_run], learning_rate[best_run], alpha[best_run])
print(params)
